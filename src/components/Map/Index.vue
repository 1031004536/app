<template>
  <div class="AMap">
    ...
    <div id="container" class="mymap">
			<el-button type="primary" @click='yesCoordinates()'>确定</el-button>
    </div>
  </div>
</template>
      ...
      geocoder.getAddress(this.center, (status, result) => {
        if (status === 'complete' && result.info === 'OK') {
          ...
          this.openInfo(this.nowAddress);
					this.parameter.center=this.center;
					this.parameter.address=this.nowAddress;
					this.$emit('getCoordinates',this.parameter);
           // 获得了有效的地址信息:
