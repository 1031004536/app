<template>
	<view>
		<view @tap="togglePopup('bottom','popup')" style="display: flex;align-items: center;font-size: 14px;">
		...
		methods: {
			...
			change(e) {
				...
				if (e.show == true) {
					...
					uni.showTabBar()
					this.$emit("send",this.selectList)
				}
			},
			tabEvent(index) {
				this.indexTab = index
			...
			provinceEvent(data, index) {
				...
				this.selectList[0].code = data.code
				this.indexTab = 1
				this.proviceShow = false
				this.cityShow = true
				this.areaShow = false
			...
			cityEvent(data, index) {
				...
				this.selectList[1].code = data.code
				this.indexTab = 2
				this.proviceShow = false
				this.cityShow = false
				this.areaShow = true
			...
			areaEvent(data, index) {
				...
				this.selectList[2].code = data.code
			}
		}
	}
</script>
