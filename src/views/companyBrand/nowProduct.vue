<template>
  <div class="app-container">
      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
          ...
          <el-form-item label="商品分类：">
            <el-cascader clearable v-model="productCategoryId" :options="productCateOptions"></el-cascader>
          </el-form-item>
    ...
    <div class="table-container">
      ...
      >
        ...
        <el-table-column label="操作" width="400" align="center">
          <template slot-scope="scope">
            <el-button size="mini" @click="handleUpdateProduct(scope.row.productId, scope.row)">编辑</el-button>
            <el-button
              size="mini"
              type="danger"
              @click="handleDelete(scope.row.productId, scope.row)"
            >删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <div style="margin-top:20px">
      <el-button type="primary" @click="addSend">采购商品</el-button>
      <el-button type="primary" @click="addProduct">采购商品池</el-button>
    </div>
    <el-dialog title="编辑货品信息" :visible.sync="editSkuInfo.dialogVisible" width="55%">
      <span>商品货号：</span>
      ...
      <el-table style="width: 100%;margin-top: 20px" :data="editSkuInfo.stockList" border>
        ...
        <el-table-column label="商品图片" width="200" align="center">
          <template slot-scope="scope">
            <single-upload v-model="scope.row.pic" style="width=100%;height:140px"></single-upload>
          </template>
        </el-table-column>
    ...
    <el-dialog title="配送列表" :visible.sync="dialogVisible" width="70%">
      <div>
        ...
        >
          ...
          <el-table-column label="操作" align="center">
            <template slot-scope="scope">
              <el-button type="primary" @click="addProductSku(scope.row)">添加</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
      <div slot="footer">
        ...
        <el-button type="primary" @click="sendSure">确定采购</el-button>
      </div>
    </el-dialog>
    <el-dialog
      title="sku"
      ...
      <el-input
        ...
        style="width: 50%;margin-left: 20px"
      >
        <el-button slot="append" icon="el-icon-search" @click="handleInnerSearchEditSku"></el-button>
      </el-input>
      <el-table style="width: 100%;margin-top: 20px" :data="skuSumList" border>
        <el-table-column label="颜色" width="200" align="center">
          <template slot-scope="scope">{{scope.row.sp1}}</template>
        </el-table-column>
        <el-table-column label="商品图片" width="200" align="center">
          <template slot-scope="scope">
            <img :src="scope.row.pic" style="width:80px;height:80px" alt="">
          </template>
        </el-table-column>
        <el-table-column label="容量" width="200" align="center">
          <template slot-scope="scope">{{scope.row.sp2}}</template>
        ...
        <el-table-column label="采购数量" width="300" align="center">
          <template slot-scope="scope">
            <el-input-number v-model="scope.row.num"  :min="1" placeholder="请输入采购数量"></el-input-number>
          </template>
        </el-table-column>
...
export default {
  ...
  watch: {
    productCategoryId: function(newValue) {
  ...
  methods: {
    ...
    handleShowSkuEditDialog(index, row) {
      ...
      fetchSkuStockList({
        ...
        skuCode: this.editSkuInfo.keyword
      }).then(response => {
    ...
    handleInnerSearchEditSku() {
      fetchSkuStockList({
        productId: this.editSkuInfo.productId,
        skuCode: this.editSkuInfo.keyword
      }).then(response => {
        this.skuNumList = response;
      });
    },
    ...
    changeStatus(row) {
      ...
      let _this = this;
      let param = {};
      param.publishStatus = row.publishStatus;
      param.productId = row.productId;
      port(
        "/product/updatePublishStatus?productId=" +
          row.productId +
          "&publishStatus=" +
          row.publishStatus,
        "post"
    ...
    sendSure() {
      ...
      if (this.sendProduct) {
        ...
        this.$confirm("是否提交数据", "提示", {
          ...
          post("/purchase/purchaseProduct", _this.sendProduct).then(res => {
            this.$message({
              message: "提交成功",
              type: "success",
              duration: 1000
            });
             _this.sendProduct.forEach(function(item, index, arr) {
                _this.multipleSelection.forEach(function(item1, index, proArr) {
                  if (item == item1) {
    ...
    addProductSku(row){
      this.skuProductId= row.productId;
       this.dialogAddVisible =true;
        fetchSkuStockList({
        productId: row.productId,
        skuCode: this.editSkuInfo.keyword
      }).then(response => {
        this.skuSumList = response;
      });
    },
    delSku(index){
      this.skuSumList.splice(index,1)
    },
    sureNum(){ 
      let numId = [];
      let sum = 0;
      this.skuSumList.forEach(item=>{
        if(item.num){
         numId.push(item.num)
         sum += item.num
        }
      });
      if(numId.length == this.skuSumList.length){
         this.multipleSelection.forEach(item=>{
           item.skuList = {};
           if(item.productId == this.skuProductId){
              item.skuList = this.skuSumList;
              item.num = sum
           }
