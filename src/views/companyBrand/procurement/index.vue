      ...
      <el-table :data="list"  style="width: 100%" v-loading="listLoading" border>
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            ...
            <el-button size="mini" @click="sendProduct(scope.row)"  v-if="scope.row.status== '待收货'">确认收货</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
  ...
  methods: {
    ...
    sendProduct(row){
      this.$confirm("是否收货", "提示", {
        ...
        post('/purchase/updatePurchaseStatus?status=3&purchaseOrderId=' + row.purchaseOrderId).then(res=>{
          this.$message({
            message: "发货成功",
            type: "success",
            duration: 1000
