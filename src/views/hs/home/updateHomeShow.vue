<template>
  <div class="app-container">
    ...
    <el-card class="form-container" shadow="never" style="width:100%;position:relative">
        ...
        <el-tab-pane label="广告图片" name="first">
          <el-container>
            ...
            <el-main>
             ...
             <div v-for="(item,index) in add.homeShowList" :key="index">
...
import { getHomeShow, addHomeShowList,option } from "@/api/hs";
...
export default {
  ...
  created() {
    this.getList()
    if (this.isEdit) {
      getHomeShow(this.$route.query.id).then(response => {
        this.add = response;
      });
    } else {
  ...
  methods: {
    ...
    addDiv() {
      ...
      if (this.add.homeShowList.length < this.add.templateId) {
        this.homeShowList.push(sub);
        this.name = "";
      }else{
         this.$message({
    ...
    panduan(){
      if(!this.upTimeflag){
               this.add.upTime =null
              }
              if(!this.downTimeflag){
                 this.add.downTime =null
              }
    },
