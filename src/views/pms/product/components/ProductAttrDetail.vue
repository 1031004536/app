  ...
  <div >
    ...
    >
      ...
      <el-form-item label="商品规格：">
        <el-card shadow="never" class="cardBg">
          <div v-for="(productAttr,idx) in selectProductAttr" :key="idx">
            {{productAttr.productAttributeName}}：
            <el-checkbox-group
        ...
        <el-table style="width: 100%;margin-top: 20px" :data="value.skuList" border>
          <el-table-column
          ...
          <el-table-column label="销售价格"  align="center">
           ...
           <el-table-column label="商品图片" width="200"  align="center">
            <template slot-scope="scope">
              <single-upload v-model="scope.row.pic"  style="height:150px"></single-upload>
            </template>
          </el-table-column>
      ...
      <!-- <el-form-item label="属性图片：" v-if="hasAttrPic">
        <el-card shadow="never" class="cardBg">
          <div v-for="(item,index) in selectProductAttrPics">
            <span>{{item.productAttributeName}}:</span>
            <single-upload
              v-model="item.pic"
      ...
      <el-form-item label="商品参数：">
        <el-card shadow="never" class="cardBg">
          <div
            ...
            :key="index"
          >
            <div class="paramInputLabel">{{item.productAttributeName}}:</div>
            <el-select
              v-if="item.inputType===1"
              class="paramInput"
              v-model="selectProductParam[index].attributeValue"
            >
              <el-option
            ...
            <el-input v-else class="paramInput" v-model="selectProductParam[index].attributeValue"></el-input>
          </div>
        </el-card>
      </el-form-item>
      ...
      <el-form-item label="商品相册：">
        <multi-upload v-model="value.picArr"></multi-upload>
      </el-form-item>
...
export default {
  ...
  components: { SingleUpload, Tinymce,MultiUpload },
  props: {
    value: Object,
    isEdit: {
      type: Boolean,
  ...
  methods: {
    ...
    getProductAttrCateList() {
      let param = { pageNum_: 1, pageSize_: 100 };
      fetchProductAttrCateList(param).then(response => {
        ...
        for (let i = 0; i < list.length; i++) {
          this.productAttributeCategoryOptions.push({
            ...
            value: list[i].productAttributeCategoryId
          });
        }
      });
    },
    getProductAttrList(type, cid) {
      let param = { pageNum_: 1, pageSize_: 100, type: type ,productAttributeCategoryId:cid};
      fetchProductAttrList(param).then(response => {
        let list = response.list;
        if (type === 0) {
          ...
          for (let i = 0; i < list.length; i++) {
            ...
            this.selectProductAttr.push({
              productAttributeId: list[i].productAttributeId,
              productAttributeName: list[i].productAttributeName,
          ...
          for (let i in this.selectProductParam) {
              for (let j in this.value.attributeList) {
                if (
                  this.selectProductParam[i].productAttributeId ==
                  this.value.attributeList[j].productAttributeId
                ) {
                  this.selectProductParam[i].attributeValue = this.value.attributeList[j].attributeValue;
                }
              }
            }
          if (this.isEdit) {
    ...
    getEditAttrOptions(id) {
      let options = [];
      for (let i = 0; i < this.value.attributeList.length; i++) {
        let attrValue = this.value.attributeList[i];
    ...
    getEditParamValue(id) {
      for (let i = 0; i < this.value.attributeList.length; i++) {
        if (id === this.value.attributeList[i].productAttributeId) {
          return this.value.attributeList[i].value;
        }
      }
    },
    ...
    handleSyncProductSkuPrice() {
      this.$confirm("将同步第一个sku的价格到所有sku,是否继续", "提示", {
        ...
        if (
          this.value.skuList !== null &&
          this.value.skuList.length > 0
        ) {
          let price = this.value.skuList[0].price;
          for (let i = 0; i < this.value.skuList.length; i++) {
            this.value.skuList[i].price = price;
          }
        }
      });
    },
    ...
    getProductSkuPic(name) {
      for (let i = 0; i < this.value.skuList.length; i++) {
        if (name === this.value.skuList[i].sp1) {
    ...
    mergeProductAttrPics() {
      for (let i = 0; i < this.selectProductAttrPics.length; i++) {
        for (let j = 0; j < this.value.skuList.length; j++) {
          if (
            this.value.skuList[j].sp1 ===
            this.selectProductAttrPics[i].productAttributeName
          ) {
    ...
    handleRemoveProductSku(index, row) {
      ...
      if(row.productSkuId){
         del('/product/deleteSku',row.productSkuId).then(response =>{
           this.$message({
            message: "删除成功",
            type: "success",
            duration: 1000
    ...
    // }
    handleFinishCommit() {
      // this.value.albumPics = this.picArr.join(',');
      if(this.value.detailHtml){
      this.mergeProductAttrValue();
