    ...
    <el-form :model="value" :rules="rules" ref="productInfoForm" label-width="120px" style="width: 600px" size="small">
      <el-form-item label="商品分类：" prop="productCategoryId">
        <el-cascader
          ...
          :placeholder="value.productCategoryName"
          ...
          for (let i = 0; i < list.length; i++) {
            ...
            if (list[i].children != null && list[i].children.length > 0) {
              for (let j = 0; j < list[i].children.length; j++) {
                children.push({label: list[i].children[j].productCategoryName, value: list[i].children[j].productCategoryId});
              }
            }
            this.productCateOptions.push({label: list[i].productCategoryName, value: list[i].productCategoryId, children: children});
          }
        });
      },
      getBrandList() {
      ...
      handleBrandChange(val) {
        for (let i = 0; i < this.brandOptions.length; i++) {
          if (this.brandOptions[i].value === val) {
            this.value.brandName = this.brandOptions[i].label;
            this.value.brandId = this.brandOptions[i].value;
            break;
          }
        }
      }
