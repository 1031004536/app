      ...
      <div style="margin-top: 15px">
        <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
          <el-form-item label="广告名称：">
            <el-input v-model="listQuery.adName" class="input-width" placeholder="广告名称"></el-input>
          </el-form-item>
...
<script>
  import {fetchList,updateStatus,deleteHomeAdvertise,option} from '@/api/homeAdvertise';
    ...
    methods: {
      ...
      handleUpdateStatus(row){
        this.$confirm('是否要修改显示状态?', '提示', {
          ...
          updateStatus(row.adId,{status:row.status}).then(response=>{
            this.getList();
            this.$message({
              type: 'success',
              message: '修改成功!'
      ...
      handleUpdate(index,row){
        this.$router.push({path: '/sms/updateAdvertise', query: {adId: row.adId}})
      },
      getList() {
        option().then(res=>{
